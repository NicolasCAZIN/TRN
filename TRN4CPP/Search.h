...
namespace TRN4CPP
{
	namespace Plugin
	{
		namespace Search
		{
			...
			{
			public:
				virtual void callback_generation(const unsigned short &condition_number, const std::vector<std::map < std::size_t, std::map<std::size_t, std::pair<float, std::vector<float>>> >> &measurements) = 0;
	...
	namespace Search
	{
		...
		std::string TRN4CPP_EXPORT retrieve(const unsigned short &condition_number, const unsigned int &simulation_number, const std::string &key);
		...
		void TRN4CPP_EXPORT		evaluate(const unsigned long long &id, const std::size_t &trial, const std::size_t &evaluation, const std::vector<float> &measurements);
