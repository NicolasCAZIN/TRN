...
static const std::string simulation_name = "SIMULATION";
...
void TRN4CPP::Simulation::compute(const std::string &filename)
{
	...
	for (auto property_element : properties)
	{
		if (boost::iequals(property_element.first, simulation_name))
		{
			...
			auto tests = _simulation.get_child(tests_attribute).get_value<std::size_t>();
			auto trials = _simulation.get_child(trials_attribute).get_value <std::size_t>();
			auto id = _simulation.get_child(id_attribute).get_value<unsigned int>();
			...
			for (auto simulation_element : _simulation)
			{
				if (boost::iequals(simulation_element.first, declaration_name))
				{
					...
					{
						if (boost::iequals(declaration_element.first, data_name))
						{
							auto _data = declaration_element.second;
							for (auto data_element : _data)
							{
								/// SEQUENCES
								if (boost::iequals(data_element.first, sequence_name))
								{
									auto _sequence = data_element.second;
								...
								/// SETS
								else if (boost::iequals(data_element.first, set_name))
								{
									auto set = data_element.second;
						...
						if (boost::iequals(configuration_element.first, model_name))
						{
							...
							for (auto model_element : _model)
							{
								...
								if (boost::iequals(model_element.first, reservoir_name))
								{
									...
									batch_size = _reservoir.get_child(batch_size_attribute).get_value<unsigned int>();
									...
									auto seed = _reservoir.get_child(seed_attribute).get_value<unsigned long>();
								...
								else if (boost::iequals(model_element.first, loop_name))
								{
									...
									else if (boost::iequals(loop_type, spatial_filter_type))
									{
										...
										auto seed = _loop.get_child(seed_attribute).get_value<unsigned long>();
								...
								else if (boost::iequals(model_element.first, scheduler_name))
								{
									...
									{
										auto seed = _scheduler.get_child(seed_attribute).get_value<unsigned long>();
										...
										auto tag = _scheduler.get_child(tag_attribute).get_value<std::string>(DEFAULT_TAG);
									...
									for (auto scheduler_element : _scheduler)
									{
										...
										if (boost::iequals(scheduler_element.first, mutator_name))
										{
											...
											{
												auto seed = _mutator.get_child(seed_attribute).get_value<unsigned long>();
				...
				else if (boost::iequals(simulation_element.first, execution_name))
				{
					...
					{
						if (boost::iequals(execution_element.first, train_name))
						{
							auto _train = execution_element.second;
						...
						else if (boost::iequals(execution_element.first, test_name))
						{
							auto _test = execution_element.second;
