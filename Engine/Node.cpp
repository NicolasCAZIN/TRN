	...
	try
	{
		...
		else
		{
			...
			switch (tag)
			{
				...
				{
					auto message = unpack<TRN::Engine::DECLARE_SEQUENCE>(locked, handle->rank, id, counter, number);
				...
				{
					auto message = unpack<TRN::Engine::CONFIGURE_LOOP_SPATIAL_FILTER>(locked, handle->rank, id, counter, number);
					if (!message.sequence.empty())
					{
						handle->cache->store(message.checksum, message.sequence);
						TRN::Engine::Message<TRN::Engine::Tag::CACHED> cached;
