...
namespace TRN
{
	namespace Engine
	{
		class ENGINE_EXPORT Broker : public TRN::Engine::Receiver
		{
		protected :
			class Handle;
		...
		public :
			Broker(const std::shared_ptr<TRN::Engine::Communicator> &communicator, const std::shared_ptr<TRN::Engine::Executor> &to_caller);
		public :
			virtual ~Broker();
		...
		public  :
			...
			void 	setup_states(const unsigned int &id, const bool &train, const bool &prime, const bool &generate);
			void 	setup_weights(const unsigned int &id, const bool &initilization, const bool &train);
			void 	setup_performances(const unsigned int &id, const bool &train, const bool &prime, const bool &generate);
			void 	setup_scheduling(const unsigned int &id);

			void	configure_begin(const unsigned int &id);
			void	configure_end(const unsigned int &id);

			void 	configure_measurement_readout_mean_square_error(const unsigned int &id, const std::size_t &batch_size);
			void  	configure_measurement_readout_frechet_distance(const unsigned int &id, const std::size_t &batch_size);
			void  	configure_measurement_readout_custom(const unsigned int &id, const std::size_t &batch_size);

			void  	configure_measurement_position_mean_square_error(const unsigned int &id, const std::size_t &batch_size);
			void  	configure_measurement_position_frechet_distance(const unsigned int &id, const std::size_t &batch_size);
			void  	configure_measurement_position_custom(const unsigned int &id, const std::size_t &batch_size);
			...
			void 	configure_loop_spatial_filter(const unsigned int &id, const std::size_t &batch_size, const std::size_t &stimulus_size, const unsigned long &seed,
			...
			void 	configure_scheduler_snippets(const unsigned int &id, const unsigned long &seed, const unsigned int &snippets_size, const unsigned int &time_budget, const std::string &tag);
	
			void 	configure_scheduler_custom(const unsigned int &id, const unsigned long &seed, const std::string &tag);

			void 	configure_mutator_shuffle(const unsigned int &id, const unsigned long &seed);
			void 	configure_mutator_reverse(const unsigned int &id, const unsigned long &seed, const float &rate, const std::size_t &size);
			void 	configure_mutator_punch(const unsigned int &id, const unsigned long &seed, const float &rate, const std::size_t &size, const std::size_t &number);

			void 	configure_mutator_custom(const unsigned int &id, const unsigned long &seed);
		...
		protected:
			...
			virtual void callback_performances(const unsigned int &id, const std::size_t &trial, const std::size_t &evaluation, const std::string &phase, const float &cycles_per_second, const float &gflops_per_second) = 0;
		...
		public :
			void notify_stimulus(const unsigned int &id, const std::size_t &trial, const std::size_t &evaluation, const std::vector<float> &stimulus, const std::size_t &rows, const std::size_t &cols);
			void notify_position(const unsigned int &id, const std::size_t &trial, const std::size_t &evaluation, const std::vector<float> &estimated_position, const std::size_t &rows, const std::size_t &cols);
			void notify_scheduler(const unsigned int &id, const std::size_t &trial, const std::vector<int> &offsets, const std::vector<int> &durations);
			void notify_mutator(const unsigned int &id, const std::size_t &trial, const std::vector<int> &offsets, const std::vector<int> &durations);
			void notify_feedforward(const unsigned int &id, const std::vector<float> &weights, const std::size_t &matrices, const std::size_t &rows, const std::size_t &cols);
			void notify_feedback(const unsigned int &id, const std::vector<float> &weights, const std::size_t &matrices, const std::size_t &rows, const std::size_t &cols);
