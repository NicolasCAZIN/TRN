...
class CSV_EXPORT Callbacks : public TRN4CPP::Plugin::Callbacks::Interface
{
private:
	class Handle;
...
private :
	void dump_csv(const unsigned long long &simulation_id, const unsigned long long &evaluation_id,
		const std::vector<std::string> &path, const std::vector<std::vector<std::string>> &values);
	void dump_matrix(const unsigned long long &simulation_id, const unsigned long long &evaluation_id,
