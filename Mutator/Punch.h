...
namespace TRN
{
	namespace Mutator
	{
		...
		public:
			Punch(const float &rate, const std::size_t &size, const std::size_t &number);
			virtual ~Punch();
		...
		public :
			static std::shared_ptr<Punch> create(const float &rate, const std::size_t &size, const std::size_t &number);
		};
	};
};
