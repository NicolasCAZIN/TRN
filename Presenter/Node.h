...
namespace TRN
{
	namespace ViewModel
	{
		namespace Node
		{
			namespace Backend
			{
				std::shared_ptr<TRN::Engine::Worker> PRESENTER_EXPORT  create(const std::shared_ptr<TRN::Engine::Communicator> &communicator,const int &rank,  const unsigned int &index);
			}

			namespace Proxy
			{
				std::shared_ptr<TRN::Engine::Proxy> PRESENTER_EXPORT  create(const std::shared_ptr<TRN::Engine::Communicator> &frontend_proxy,
