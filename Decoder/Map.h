...
namespace TRN
{
	namespace Decoder
	{
		class DECODER_EXPORT Map : public Kernel
		{
		private:
			class Handle;
		...
		public:
			Map(const std::shared_ptr<TRN::Backend::Driver> &driver,
		...
		public:
			static std::shared_ptr<TRN::Decoder::Map> create(const std::shared_ptr<TRN::Backend::Driver> &driver,
				const std::size_t &batch_size, const std::size_t &stimulus_size,
				const std::size_t &rows, const std::size_t &cols,
				const std::pair<float, float> &arena_x, const std::pair<float, float> &arena_y,
				const float &sigma, const float &radius, const float &angle, const float &scale, const unsigned long &seed,
